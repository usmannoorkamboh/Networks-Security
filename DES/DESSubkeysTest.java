...
public class DESSubkeysTest {
    public static byte[][] getSubkeys(byte[] theKey) throws Exception {
        ...
        int[] temp2 = {14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};

        byte[] k = selectBits(theKey, temp);
        byte[][] c = new byte[17][];
        byte[][] d = new byte[17][];

        c[0] = selectBits(k, 0, 28);
        d[0] = selectBits(k, 28, 28);

        for (int i = 0; i < 16; i++) {
            c[i + 1] = rotateLeft(c[i], 28, SHIFTS[i]);
            d[i + 1] = rotateLeft(d[i], 28, SHIFTS[i]);
            byte[] x = concatenateBits(c[i + 1], 28, d[i + 1], 28);
            output[i] = selectBits(x, temp2);
        }
        return output;
    }
    ...
    private static byte[] rotateLeft(byte[] in, int len, int step) {
        ...
        return out;
		   ...
		   /* Bits Concatenation for next permuted choice 2 */
    ...
    private static byte[] concatenateBits(byte[] a, int aLen, byte[] b,
        ...
        return out;
		   ...
		   /* Dividing in two Halves (28/28) Bits */
