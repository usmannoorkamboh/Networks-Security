...
public class DESCipherTest {
    public static byte[] cipherBlock(byte[] theMsg, byte[][] subKeys)
        ...
        theMsg = selectBits(theMsg, IP);
        byte[][] r = new byte[17][];
        byte[][] l = new byte[17][];

        l[0] = selectBits(theMsg, 0, 32);
        r[0] = selectBits(theMsg, 32, 32);

        for (int i = 0; i < 16; i++) {
            byte[] temp = f(r[i], subKeys[i]);
            r[i + 1] = doXORBytes(l[i], temp);
            l[i + 1] = r[i];

        }

        output = concatenateBits(r[16], 32, l[16], 32);
        output = selectBits(output, FP);
        return output;
    }


    //Function for encryption
    private static byte[] f(byte[] data, byte[] key) {
        byte[] output;

        byte[] x = selectBits(data, E);
        x = doXORBytes(x, key);
        x = substitution6x4(x);
        output = selectBits(x, P);
        return output;
    }


    //Function for decryption
    public static byte[] cipherBlockDecrypt(byte[] theMsg, byte[][] subKeys)
            throws Exception {
        byte[] output = theMsg;
        byte[] temp;
        theMsg = selectBits(theMsg, IP);
